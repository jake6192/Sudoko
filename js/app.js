    ...
    ********************************************************/
    ...
    this.assignRandomValues = (populatePDfList) => {
      ...
      if(populatePDfList) { // populatePDfList - Development tool to generate random games to fill predefinedGames[[]] with. //
        let len = predefinedGames.length;
        predefinedGames.push(BOARD.GAME.saveValuesToJSON());
        predefinedGames = predefinedGames.map((f) => JSON.stringify(f)).filter((e, i) => predefinedGames.map((f) => JSON.stringify(f)).indexOf(e) === i).map((f) => JSON.parse(f)); // Remove duplcate items from the arra. //
  ...
  constructor() {
    ...
    this.permutePuzzle = function(puzzle, perms) {
      for(var i=0; i<perms.length; i++)
        puzzle[i] = this.permuteRow(puzzle[i], perms[i]);
      return puzzle;
    };
    ...
    this.makeSolution = function(row) {
      let permutationOffsets = arr_shuffle([[0, 3, 6], [2, 5, 8], [1, 4, 7]]);
      let puzzle = this.permutePuzzle(this.makeRows(row), arr_shuffle(permutationOffsets[0]).concat(arr_shuffle(permutationOffsets[1])).concat(arr_shuffle(permutationOffsets[2])));
    ...
    this.start = () => {
      this.cells = [];
      let startingRow = arr_shuffle([1,2,3,4,5,6,7,8,9]);
